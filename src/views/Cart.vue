        ...
        <div class="item-list-wrap">
          <div class="cart-item">
            ...
            <transition-group tag="ul" class="cart-item-list">
              <li v-for="item in cartList" :key="item.productId" class="cartItem">
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{ item.productNum }}</div>
                </div>
          ...
          <p class="no-shopping" v-if="!cartList.length">暂时没有商品，快去购物吧~</p>
        </div>
        <div class="cart-foot-wrap" v-if="cartList.length">
...
export default class Cart extends Vue {
  ...
  public created() {
    this.$store.commit("changeBreadText", "My Cart");
  }
  ...
  public async delCart() {
    ...
    if (data.status === "0") {
      const i = this.cartList.findIndex(
