<template>
  <div class="checkout-page">
              ...
              >
                ...
                <div class="addr-opration addr-del">
                  <a
                    href="javascript:;"
                    class="addr-del-btn"
                    @click="delAddressModel=true;delAddressId=item.addressId"
                  >
    ...
    <modal :mdShow="addAddressModal" @closeModal="closeModal">
      <p slot="title" style="font-size: 20px;margin-bottom: 10px">add Address</p>
      <div slot="message">
        <label
          style="width: 60%; margin-bottom: 5px; display: flex; justify-content: space-between; align-items:
        center"
        >userName:
          <input type="text" style="padding: 5px" v-model="addressInfo.userName">
        </label>
        <label
          style="width: 60%; margin-bottom: 5px; display: flex; justify-content: space-between; align-items:
        center"
        >streetName:
          <input type="text" style="padding: 5px" v-model="addressInfo.streetName">
        </label>
        <label
          style=" width: 60%; margin-bottom: 5px; display: flex; justify-content: space-between; align-items:
        center"
        >postCode:
          <input type="text" style="padding: 5px" v-model="addressInfo.postCode">
        </label>
        <label
          style=" width: 60%; margin-bottom: 5px; display: flex; justify-content: space-between; align-items:
        center"
        >tel:
          <input type="text" style="padding: 5px" v-model="addressInfo.tel">
        </label>
      </div>
      <div slot="btnGroup">
...
export default class Address extends Vue {
  ...
  public created(): void {
    this.$store.commit("changeBreadText", "My Address");
    this.getAddressList();
  }
  ...
  public async addAddress() {
    for (const item in this.addressInfo) {
      if (
        item !== "addressId" &&
        item !== "isDefault" &&
        !Boolean(this.addressInfo[item])
      ) {
        return;
      }
    }
    this.addressInfo.addressId = `${Date.now()}`;
    const response = (await this.axios.post("/users/addAddress", {
  ...
  public async delAddress() {
    ...
    if (response.status === "0") {
      const delAddressIndex = this.addressList.findIndex(
        (item: any) => item.addressId === this.delAddressId
      );
